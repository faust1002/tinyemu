...
cfg_options parse(int argc, char** argv)
{
    ...
    op.add_options()
        ...
        ("logfile,l", po::value<std::string>(&options.log_file)->default_value("stdout"))
        ("severity,s", po::value<std::string>(&options.log_severity)->default_value("info"))
        ("max,m", po::value<int>(&options.log_size)->default_value(15))
        ("port_id,p", po::value<uint8_t>(&options.port_id)->required())
    ;
